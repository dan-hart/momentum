...
pub fn register_global(app: &MomentumApplication) {
    glib::spawn_future_local(glib::clone!(
        ...
        async move {
            ...
            let Ok(session) = gs.create_session(Default::default()).await else {
                return;
            };
            let wanted = [
            ...
            if gs
                ...
                .is_err()
            {
                tracing::info!("GlobalShortcuts portal unavailable or declined");
                return;
            }
            let Ok(mut activated) = gs.receive_activated().await else {
                return;
            };
